import java.io.File;
import java.io.FileWriter;
import java.io.IOException;
import java.util.Scanner;

public class Deletion {

    void accCloseFun(int accountNo, String fileName) throws IOException {
        System.out.println("Are you sure want to delete your account?");
        System.out.println("Type 1: Yes");
        System.out.println("Type 2: No");
        Scanner tscanner = new Scanner(System.in);
        int conf = tscanner.nextInt();
        if (conf == 2) {
            Main.menu(accountNo);
        } else if (conf != 1) {
            System.out.println("Incorrect! Choose a valid option again.\n");
            accCloseFun(accountNo, fileName);
        }
        delLine(accountNo, fileName);
    }

    void delLine(int accountNo, String fileName) throws IOException {

        File file = new File(fileName);
        String newInfo = "";
        Scanner scanner = new Scanner(file);
        while (scanner.hasNextLine()) {
            String line = scanner.nextLine();
            String[] subLine = line.split(" ");
            if (accountNo == Integer.parseInt(subLine[0])) {
                continue;
            }
            String newLine = "";
            for (int x = 0; x < subLine.length; x++) {
                newLine += subLine[x] + " ";
            }
            newInfo += newLine.trim() + "\n";

        }

        if (newInfo.endsWith("\n")) {
            newInfo = newInfo.substring(0, newInfo.length() - 1);
        }

        FileWriter writer = new FileWriter(fileName);
        writer.write(newInfo);
        writer.close();
        scanner.close();

    }

}
